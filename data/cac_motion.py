...
def warp_cardiac_state(
    ...
    """心臓mask内だけへ局所的な3D affine motionを適用する。"""
    ...
    for z0 in range(0, clean.shape[0], max(1, int(chunk_depth))):
        ...
        coords = np.stack(
            [
                zz + weight * (source_z - zz),
                yy + weight * (source_y - yy),
                xx + weight * (source_x - xx),
            ],
            axis=0,
        )
...
def _sample_phases(num_samples: int, config, params, rng) -> np.ndarray:
    heart_period_ms = 60000.0 / max(float(params["heart_rate_bpm"]), 1.0)
    angular_range = float(
        _get(_get(config, "parallel_fbp", {}), "angular_range_deg", 180.0)
    )
...
def simulate_cac_motion(
    ...
    if identity or severity == 0.0:
        ...
        if simulator == "image_blend":
            ...
            raise ValueError(
                "cac_motion.simulatorはimage_blend/parallel_fbpで指定してください: "
                f"{simulator}"
            )
